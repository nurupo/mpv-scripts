-- Modified ytdl_hook.lua of mpv 0.32.0 to prevent a crash when using yt-dlp
-- The original was taken from https://github.com/mpv-player/mpv/blob/v0.32.0/player/lua/ytdl_hook.lua
...
function run_ytdl_hook(url)
    ...
    if (json == nil) then
        msg.error("failed to parse JSON data: " .. (err or "null json"))
        check_version(ytdl.path)
        return
    end
